...
while isConsult:
  ...
  if selection == 1 :
    ...
    for i in range(len(products_list)):
      ...
      search_key = "name" if produt_find_by == 1 else "code" if produt_find_by == 2 else ""
      if search_key:
        founed_product = input(f'Ingrese el {"nombre" if search_key == "name" else "código"} del producto a buscar: ').lower()
        print("\n--- Actualizar Cantidad Productos---")
        amout_product_update = 0
        for i_update in range(len(products_list)):
          search = products_list[i_update]
          if str(search[search_key]).lower() == founed_product:
